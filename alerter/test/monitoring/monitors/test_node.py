...
from alerter.src.alerting.alerts.alerts import \
    FoundLiveArchiveFullNodeAgainAlert
...
from alerter.src.utils.exceptions import \
    ...
    NoLiveArchiveFullNodeConnectedWithAnApiServerException
from alerter.src.utils.redis_api import RedisApi
from alerter.src.utils.scaling import scale_to_tera
from alerter.src.utils.types import NONE
...
GET_WEB_SOCKETS_FUNCTION = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.' \
    'get_web_sockets_connected_to_an_api'

PING_NODE_FUNCTION = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.ping_node'

GET_SYSTEM_HEALTH_FUNCTION = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.get_system_health'

GET_FINALIZED_HEAD_FUNCTION = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.get_finalized_head'

GET_HEADER_FUNCTION = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.get_header'

GET_BLOCK_HASH_FUNCTION = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.get_block_hash'

GET_SLASH_AMOUNT_FUNCITON = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.get_slash_amount'

GET_SESSION_VALIDATORS_FUNCTION = \
    'alerter.src.monitoring.monitors.node.PolkadotApiWrapper.' \
    'get_session_validators'

GET_STAKERS_FUNCTION = 'alerter.src.monitoring.monitors.node.' \
                       'PolkadotApiWrapper.get_stakers'

GET_COUNCIL_MEMBERS_FUNCTION = 'alerter.src.monitoring.monitors.node.' \
                               'PolkadotApiWrapper.get_council_members'

GET_CURRENT_ELECTED_FUNCTION = 'alerter.src.monitoring.monitors.node.' \
                               'PolkadotApiWrapper.get_current_elected'

GET_CURRENT_INDEX_FUNCTION = 'alerter.src.monitoring.monitors.node.' \
                             'PolkadotApiWrapper.get_current_index'

GET_AUTHORED_BLOCKS_FUNCTION = 'alerter.src.monitoring.monitors.node.' \
                               'PolkadotApiWrapper.get_authored_blocks'

GET_DISABLED_VALIDATORS_FUNCTION = 'alerter.src.monitoring.monitors.node.' \
                                   'PolkadotApiWrapper.get_disabled_validators'

DATA_SOURCE_INDIRECT_PATH = 'alerter.src.monitoring.monitors.node.' \
                            'NodeMonitor.data_source_indirect'

DATA_SOURCE_ARCHIVE_PATH = 'alerter.src.monitoring.monitors.node.' \
                           'NodeMonitor.data_source_archive'
...
class TestNodeMonitorWithoutRedis(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.full_node_monitor = NodeMonitor(
            ...
            self.archive_alerts_disabled, self.full_nodes,
        ...
        self.dummy_full_node_4 = Node(name=self.dummy_node_name_4,
                                      ...
                                      is_archive_node=True,
                                      internal_conf=TestInternalConf)
        self.dummy_full_node_5 = Node(name=self.dummy_node_name_5,
    ...
    def test_data_source_indirect_chooses_an_online_node_connected_to_the_API(
        ...
        self.validator_monitor._all_full_nodes = [self.dummy_full_node_1,
    ...
    @patch(PING_NODE_FUNCTION, return_value=None)
    @patch(GET_WEB_SOCKETS_FUNCTION)
    def test_data_source_archive_chooses_an_online_archive_node_connected_to_the_API(
            self, mock_get_web_sockets, _) -> None:
        self.dummy_full_node_1.set_as_down(self.channel_set, self.logger)
        self.dummy_full_node_3.set_as_down(self.channel_set, self.logger)
        self.validator_monitor._all_full_nodes = [self.dummy_full_node_1,
