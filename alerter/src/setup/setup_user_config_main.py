...
from alerter.src.alerting.alert_utils.telegram_bot_api import TelegramBotApi
from alerter.src.alerting.alert_utils.twilio_api import TwilioApi
from alerter.src.setup.setup_user_config_main_tests import \
    test_telegram_alerts, TestOutcome, test_email_alerts, test_twilio_alerts, \
...
from alerter.src.utils.user_input import yn_prompt
...
def setup_general(cp: ConfigParser) -> None:
    ...
    print('The first step is to set a unique identifier for the alerter. This '
          ...
          'of the PANIC alerter, to avoid any possible Redis clashes. The '
...
def setup_redis(cp: ConfigParser) -> None:
    ...
    while True:
        print('You will now be asked to input the IP of the Redis server.\n'
              'If you will be using PANIC as a Docker container, do not use '
        ...
        print('You will now be asked to input the port of the Redis server.\n'
              'If you will be using PANIC as a Docker container, you should '
              'leave the port as the default. Otherwise, you must run the '
              'Redis Docker using -p <port>:6379.')
...
def setup_api(cp: ConfigParser) -> None:
    print('==== Custom JS API')
    print('The custom JS API is used by the alerter to get data from the '
          'nodes. It is important that before running both the alerter and '
          'this setup, the custom JS API server is set up and running.')

    if is_already_set_up(cp, 'api') and \
            not yn_prompt('The custom JS API is already set up. Do you wish to '
                          'replace the current config? (Y/n)\n'):
        return
    ...
    while True:
        print('You will now be asked to input the JS API\'s address\n'
              'If you will be using PANIC as a Docker container, do not use '
              'localhost, instead use the full IP address (local or external) '
        ...
        polkadot_api_endpoint = input('Please insert the JS API\'s address: '
