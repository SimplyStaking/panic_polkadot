...
from alerter.src.utils.config_parsers.user import NodeConfig
...
def get_node(nodes_so_far: List[NodeConfig],
    ...
    while True:
        ...
        if ws_url in web_sockets_connected_to_api:
            ...
            if not yn_prompt(
                    'Could not connect with the custom JS API at web socket '
...
def setup_nodes(cp: ConfigParser, api_endpoint: str) -> None:
    ...
    print('To produce alerts, the alerter needs something to monitor! The list '
          ...
          'as a data source to monitor a node\'s state indirectly. You may '
          ...
          'slashing), the alerter needs blockchain data from the past. You do '
          'not need any archive nodes to run PANIC, but for archive monitoring '
          'to be enabled you must have at least one.')
    ...
    while True:
        ...
        while True:
            try:
                ...
                if not yn_prompt('Could not connect with the custom JS API '
                                 'at \'{}\'. Please make sure that the API is '
                                 'running at the provided IP before proceeding '
                                 'further. Do you want to try again? (Y/n)\n'
                                 ''.format(api_endpoint)):
                    return
    ...
    for i, node in enumerate(nodes):
        ...
        cp[section]['include_in_blockchain_monitor'] = \
            'true' if node.include_in_blockchain_monitor else 'false'
        cp[section]['stash_account_address'] = \
            node.stash_account_address
