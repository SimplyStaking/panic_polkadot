...
from alerter.src.utils.parsing import parse_int_from_string
from alerter.src.utils.redis_api import RedisApi
from alerter.src.utils.scaling import scale_to_pico
...
class NodeMonitor(Monitor):
    ...
    def __init__(self, monitor_name: str, channels: ChannelSet,
                 ...
                 archive_alerts_disabled: bool, all_full_nodes: List[Node],
    ...
    # The data_source_indirect function returns a full node for the indirect
    # monitoring. Since indirect monitoring does not require data from past
    # chain state, the data_source_indirect function may return a full node
    # which is not an archive node.
    @property
    ...
    # The data_source_archive function returns a full node for the archive
    # monitoring. Since archive monitoring requires data from past chain state,
    # the data_source_archive function returns only full nodes which are
    # archive nodes.
    @property
    def data_source_archive(self) -> Node:
        ...
        # Get one of the full nodes to use as data source
        for n in self._all_full_nodes:
            if n.ws_url in nodes_connected_to_an_api and not n.is_down and \
                    n.is_archive_node:
                self.last_full_node_used = n
                self._data_wrapper.ping_node(n.ws_url)
                return n
        raise NoLiveArchiveFullNodeConnectedWithAnApiServerException()
    ...
    def _monitor_archive_state(self) -> None:
        ...
        if self._no_live_archive_node_alert_sent:
            self._no_live_archive_node_alert_sent = False
            self.channels.alert_info(FoundLiveArchiveFullNodeAgainAlert(
                self.monitor_name))
